...
Все внешние API-методы описаны через Swagger. В развернутой системе доступен Swagger UI по адресу */swagger-ui.html*, полная разметка с описанием методов, принимаемых и отдаваемых объектов доступна по адресу *api/v1/api-docs*. Какие-то из методов доступны администраторам, какие-то только владельцам ресурса, какие-то и врачам в случае, если пациент-владелец к ним прикреплен. Выделяются роли больного, врача, администратора и суперпользователя.

На данный момент для авторизации используются сессии, поэтому для большинства методов необходима отправка куки JSESSIONID с ID сессии, которая получается при успешном логине по адресу */api/v1/auth/process-login*. Также POST, PUT, DELETE методы (в том числе для регистрации) требуют в запросе CSRF-токен в заголовке X-XSRF-TOKEN и в куки XSRF-TOKEN (токены в заголовке и в куки должны быть одинаковые). Первоначально его можно получить GET-запросом по адресу */api/v1/auth/csrf*. Выдается как в JSON-теле ответа, так и в виде куки XSRF-TOKEN. После логина токен может поменяться.
...
- различные TODO в коде по его переорганизации;
