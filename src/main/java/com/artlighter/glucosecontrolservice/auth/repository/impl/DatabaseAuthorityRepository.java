...
public class DatabaseAuthorityRepository implements AuthorityRepository {
    ...
    public Map<Authority, Boolean> getRoleAuthorities(Role role) {
        ...
        String queryString = String.format("FROM RoleAuthorityEntity WHERE role='%s'", role.name());
        List<RoleAuthorityEntity> resultList = entityManager.createQuery(queryString, RoleAuthorityEntity.class)
                .getResultList();
    ...
    @Embeddable
    private static class RoleAuthorityID {
