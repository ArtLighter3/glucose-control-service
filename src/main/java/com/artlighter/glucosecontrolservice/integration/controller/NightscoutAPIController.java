...
@RestController
@RequestMapping("/nightscout/{userId}/api/v1")
public class NightscoutAPIController {
    ...
    @PostMapping("/entries")
    @PreAuthorize("@nightscoutAuthUtils.hasAccessToNightscoutApi(#userId, #apiSecret)")
    public List<NightscoutEntryDTO> postEntry(@PathVariable int userId,
        ...
        rejected.addAll(nightscoutService.addGlucoseEntries(toAdd, userId, true));
        return rejected;
    }

    @PostMapping("/treatments")
    @PreAuthorize("@nightscoutAuthUtils.hasAccessToNightscoutApi(#userId, #apiSecret)")
    public List<NightscoutTreatmentDTO> postTreatment(@PathVariable int userId,
                                                      @RequestHeader("api-secret") String apiSecret,
        ...
        rejected.addAll(nightscoutService.addTreatments(toAdd, userId, true));
        return rejected;
    }

    @PutMapping("/treatments")
    @PreAuthorize("@nightscoutAuthUtils.hasAccessToNightscoutApi(#userId, #apiSecret)")
    public List<NightscoutTreatmentDTO> putTreatment(@PathVariable int userId,
                                                      @RequestHeader("api-secret") String apiSecret,
                                                      @RequestBody NightscoutTreatmentDTO treatment) {
        return postTreatment(userId, apiSecret, List.of(treatment));
    }
