...
@Import(DelegatingCommonDiaryEntryDAO.class)
public class DelegatingCommonDiaryEntryDAOTests {
    @MockitoBean
    ...
    @Autowired
    private DelegatingCommonDiaryEntryDAO dao;
    ...
    public void getAllOfTypeBetweenDates_GetsCorrectType_ReturnsListOfEntriesOfType() {
        ...
        when(glucoseRepository.getAllByProfileIdAndCommitedAtBetween(anyInt(), eq(from), eq(to), notNull()))
                .thenReturn(new SliceImpl<>(expected));
        when(glucoseRepository.getAllByProfileIdAndCommitedAtBetween(anyInt(), eq(from), eq(to)))
                .thenReturn(expected);

        List<DiaryEntry> actual = dao.getAllOfTypeBetweenDates(DiaryEntryType.GLUCOSE_ENTRY,
                0, from, to, Pageable.unpaged()).getContent();
        ...
        when(insulinRepository.getAllByProfileIdAndCommitedAtBetween(anyInt(), eq(from), eq(to), notNull()))
                .thenReturn(new SliceImpl<>(expected2));
        when(insulinRepository.getAllByProfileIdAndCommitedAtBetween(anyInt(), eq(from), eq(to)))
                .thenReturn(expected2);

        List<DiaryEntry> actual2 = dao.getAllOfTypeBetweenDates(DiaryEntryType.INSULIN_ENTRY,
                0, from, to, Pageable.unpaged()).getContent();
    ...
    public void getAllOfTypeBetweenDates_GetsNullType_ReturnsListOfAllEntries() {
        ...
        List<InsulinEntry> insulinEntries = filter(expected, from, to, DiaryEntryType.INSULIN_ENTRY)
                .stream().map((entry) -> (InsulinEntry) entry).collect(Collectors.toList());
        when(commonDiaryEntryRepository.getAllByProfileIdAndCommitedAtBetween(eq(0), eq(from), eq(to), notNull()))
                .thenReturn(new SliceImpl<>(expected));

        List<DiaryEntry> actual =
                dao.getAllOfTypeBetweenDates(null, 0, from, to, Pageable.unpaged()).getContent();
    ...
    public void saveOrUpdate_GetsCorrectEntity_CallsCorrectRepositoryToSaveOrUpdate() {
        ...
        DiaryEntry diaryEntry = dao.saveOrUpdate(glucoseEntry);
        ...
        diaryEntry = dao.saveOrUpdate(insulinEntry);
    ...
    public void saveOrUpdate_DiaryEntryIsNullOrDoesNotHaveIdFields_ThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> dao.saveOrUpdate(null));
        ...
        assertThrows(IllegalArgumentException.class, () -> dao.saveOrUpdate(entry));
    }

    @Test
    public void deleteById_GetsCorrectEntity_CallsCorrectRepositoryToRemove() {
        DiaryEntry.DiaryEntryID glucoseEntryId =
                new DiaryEntry.DiaryEntryID(0, Instant.now(), DiaryEntryType.GLUCOSE_ENTRY);
        DiaryEntry.DiaryEntryID insulinEntryId =
                new DiaryEntry.DiaryEntryID(0, Instant.now(), DiaryEntryType.INSULIN_ENTRY);

        dao.deleteById(glucoseEntryId);
        dao.deleteById(insulinEntryId);
    ...
    @Test
    public void deleteById_IdIsNullOrDoesNotHaveIdFields_ThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> dao.deleteById(null));
        assertThrows(IllegalArgumentException.class, () ->
                dao.deleteById(new DiaryEntry.DiaryEntryID(0, Instant.now(), null)));
        assertThrows(IllegalArgumentException.class, () ->
                dao.deleteById(new DiaryEntry.DiaryEntryID(0, null, DiaryEntryType.INSULIN_ENTRY)));
        assertThrows(IllegalArgumentException.class, () ->
                dao.deleteById(new DiaryEntry.DiaryEntryID(0, null, null)));
...
//                new DiaryEntry.DiaryEntryID(null, null)));
    ...
    public void exists_GetsCorrectEntity_CallsCorrectRepositoryAndReturnsCorrectBoolean() {
        ...
        when(insulinRepository.existsById(new DiaryEntry.DiaryEntryID(existentInsulinEntry.getProfileId(),
                existentInsulinEntry.getCommitedAt(), DiaryEntryType.INSULIN_ENTRY))).thenReturn(true);
        when(insulinRepository.existsById(new DiaryEntry.DiaryEntryID(nonexistentInsulinEntry.getProfileId(),
                nonexistentInsulinEntry.getCommitedAt(), DiaryEntryType.INSULIN_ENTRY))).thenReturn(false);
        when(glucoseRepository.existsById(new DiaryEntry.DiaryEntryID(existentGlucoseEntry.getProfileId(),
                existentGlucoseEntry.getCommitedAt(), DiaryEntryType.GLUCOSE_ENTRY))).thenReturn(true);
        when(glucoseRepository.existsById(new DiaryEntry.DiaryEntryID(nonexistentGlucoseEntry.getProfileId(),
                nonexistentGlucoseEntry.getCommitedAt(), DiaryEntryType.GLUCOSE_ENTRY))).thenReturn(false);

        assertTrue(dao.exists(existentGlucoseEntry));
        assertTrue(dao.exists(existentInsulinEntry));
        assertFalse(dao.exists(nonexistentGlucoseEntry));
        assertFalse(dao.exists(nonexistentInsulinEntry));
    ...
    public void exists_DiaryEntryIsNullOrDoesNotHaveIdFields_ThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> dao.exists(null));
        ...
        entry.setCommitedAt(null);
        entry.setProfileId(0);
        assertThrows(IllegalArgumentException.class, () -> dao.exists(entry));
    }
